...
if __name__ == "__main__":
    ...
    sym = get_resnet_model('pretrained_models/resnet-34', 0, "bn1_output")
    ...
    train_data = get_iterator(path='DATA_rec/cat_small_xiao.rec', data_shape=(3, 224, 224), label_width=7 * 7 * 5, batch_size=5,
                              shuffle=True)
    val_data = get_iterator(path='DATA_rec/cat_small_xiao.rec', data_shape=(3, 224, 224), label_width=7 * 7 * 5, batch_size=5)
    ...
    # save model
    checkpoint = mx.callback.do_checkpoint('models/cat_xiao_small',period = 50)
    ...
    mod.fit(train_data=train_data,
            ...
            allow_missing=True,
            batch_end_callback=[mx.callback.Speedometer(batch_size=5, frequent=10, auto_reset=False)],
